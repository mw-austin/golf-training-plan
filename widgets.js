/* ============================================================
   ...
   - drill checklist (drills page)
   - phase tracker (plan page)
   - KPI tracker + chart (kpis page)
   ============================================================ */
...
(function () {
  ...
  function initDrillChecklist() {
    ...
    if (reset) {
      reset.addEventListener("click", () => {
        if (!confirm("Reset all drill checkboxes for this session?")) return;
        saveChecks({});
        document.querySelectorAll(".drill-checkbox").forEach((cb) => { cb.checked = false; });
        updateDrillSummary();
      });
    }
    updateDrillSummary();
  ...
  function initPhaseTracker() {
    ...
    container.addEventListener("click", (e) => {
      ...
      if (state.done.includes(id)) {
        ...
        // mark done
        state.done.push(id);
        const next = window.GolfSite.phases.find((p) => p.id === id + 1);
        state.active = next ? next.id : id;
      } else {
        // make active
        state.active = id;
      }
    ...
    function render() {
      const state = window.GolfSite.getPhaseState();
      container.innerHTML = window.GolfSite.phases
        .map((p) => {
          let status = "future";
          ...
          return `
            <div class="phase-card" data-phase="${p.id}" data-status="${status}" role="button" tabindex="0">
  ...
  function getLog() {
    try { return JSON.parse(localStorage.getItem(KPI_KEY)) || []; } catch (_) { return []; }
  }
  ...
  function initKpiTracker() {
    ...
    function render() {
        ...
        tableBody.innerHTML = log
          ...
          .map(
            ...
            <tr>
              ...
              <td>${e.gir ?? "—"}</td>
              <td>${e.score ?? "—"}</td>
              <td><button class="delete-btn" data-delete="${log.length - 1 - idx}" aria-label="Delete">×</button></td>
            </tr>`
          )
          .join("");
      }
      ...
      const metricLabels = {
        driverFtp: "Driver Face-to-Path (°)",
        ironAoa: "7-Iron AoA (°)",
        ironSmash: "7-Iron Smash Factor",
      ...
      if (chartCanvas && data.length > 0 && window.Chart) {
        ...
        chart = new Chart(chartCanvas, {
          ...
          options: {
            ...
            plugins: {
              legend: { labels: { color: text, font: { family: "Inter, sans-serif" } } },
              tooltip: { titleColor: text, bodyColor: text, backgroundColor: styles.getPropertyValue("--surface").trim(), borderColor: border, borderWidth: 1 },
            },
            scales: {
    ...
    form.addEventListener("submit", (e) => {
      ...
      const entry = {
        date: fd.get("date"),
        driverFtp: fd.get("driverFtp") || null,
        ironAoa: fd.get("ironAoa") || null,
        ironSmash: fd.get("ironSmash") || null,
        fairways: fd.get("fairways") || null,
        gir: fd.get("gir") || null,
    ...
    if (exportBtn) {
      exportBtn.addEventListener("click", () => {
        ...
        const headers = ["date","driverFtp","ironAoa","ironSmash","fairways","gir","score"];
        const csv = [headers.join(",")].concat(
          log.map((e) => headers.map((h) => e[h] ?? "").join(","))
        ).join("\n");
        const blob = new Blob([csv], { type: "text/csv" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url; a.download = "golf-kpi-log.csv"; a.click();
    ...
    if (clearBtn) {
      clearBtn.addEventListener("click", () => {
        if (!confirm("Delete all KPI log entries? This can't be undone.")) return;
        saveLog([]);
        render();
      });
    }
